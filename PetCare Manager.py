...
class PetCareApp:
    ...
    def kreiraj_stil(self):
        ...
        stil.configure("TButton", background="#b3d9ff", foreground="black",
    ...
    def kreiraj_sucelje(self):
        ...
        lijevi_okvir.rowconfigure(3, weight=1)

        tk.Label(lijevi_okvir, text="Ljubimci", font=("Segoe UI", 12, "bold"), bg="#cce0ff").pack(pady=5)

      
        pretraga_frame = tk.Frame(lijevi_okvir, bg="#cce0ff")
        pretraga_frame.pack(fill=tk.X, padx=10)

        tk.Label(pretraga_frame, text="🔍 Pretraga:", bg="#cce0ff").pack(side=tk.LEFT)
        self.search_var = tk.StringVar()
        search_entry = tk.Entry(pretraga_frame, textvariable=self.search_var)
        search_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        self.search_var.trace("w", lambda *args: self.filtriraj_listu())

        
        sort_frame = tk.Frame(lijevi_okvir, bg="#cce0ff")
        sort_frame.pack(fill=tk.X, padx=10, pady=5)

        tk.Label(sort_frame, text="Sortiraj po:", bg="#cce0ff").pack(side=tk.LEFT)
        self.sort_var = tk.StringVar(value="Ime")
        sort_combo = ttk.Combobox(sort_frame, textvariable=self.sort_var, values=["Ime", "Vrsta", "Dob"], width=7)
        sort_combo.pack(side=tk.LEFT, padx=5)

        ttk.Button(sort_frame, text="↕ Sortiraj", command=self.sortiraj_ljubimce).pack(side=tk.LEFT)
        ...
        self.lbl_info = tk.Label(desni_okvir, text="", font=("Segoe UI", 11, "bold"),
                                 bg="#f2f7ff", justify="center")
    ...
    def filtriraj_listu(self):
        pojam = self.search_var.get().lower()
        self.lista.delete(0, tk.END)

        for ljub in self.ljubimci:
            if pojam in ljub.ime.lower() or pojam in ljub.vrsta.lower():
                self.lista.insert(tk.END, str(ljub))


    def sortiraj_ljubimce(self):
        kriterij = self.sort_var.get()

        if kriterij == "Ime":
            self.ljubimci.sort(key=lambda x: x.ime.lower())
        elif kriterij == "Vrsta":
            self.ljubimci.sort(key=lambda x: x.vrsta.lower())
        elif kriterij == "Dob":
            self.ljubimci.sort(key=lambda x: x.dob)
